...
func AllResources() map[string]Resource {
	return map[string]Resource{
		...
		"Secure::S3::SecureBucket":                                    &s3.SecureBucket{},
	}
}
...
// GetAllS3SecureBucketResources retrieves all s3.SecureBucket items from an AWS CloudFormation template
func (t *Template) GetAllS3SecureBucketResources() map[string]*s3.SecureBucket {
	results := map[string]*s3.SecureBucket{}
	for name, untyped := range t.Resources {
		switch resource := untyped.(type) {
		case *s3.SecureBucket:
			results[name] = resource
		}
	}
	return results
...
// GetS3SecureBucketWithName retrieves all s3.SecureBucket items from an AWS CloudFormation template
// whose logical ID matches the provided name. Returns an error if not found.
func (t *Template) GetS3SecureBucketWithName(name string) (*s3.SecureBucket, error) {
	if untyped, ok := t.Resources[name]; ok {
		switch resource := untyped.(type) {
		case *s3.SecureBucket:
			return resource, nil
	...
	return nil, fmt.Errorf("resource %q of type s3.SecureBucket not found", name)
}
